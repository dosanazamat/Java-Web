...
(function () {
  ...
  function renderVariantSelect() {
    ...
    const card = el('section', { class: 'card' },
      ...
      el('p', { class: 'subtitle' },
        `Всего ${subjectData.variants.length} вариантов по 40 вопросов. ` +
        `Или возьми режим «Микс» — 40 случайных вопросов из всех вариантов.`
      ),

      el('div', { class: 'random-row' },
        el('button',
          { class: 'random-tile', type: 'button', onclick: () => startRandomQuiz(40) },
          el('div', { class: 'random-tile-icon' }, '🎲'),
          el('div', { class: 'random-tile-body' },
            el('div', { class: 'random-tile-title' }, 'Микс — 40 случайных'),
            el('div', { class: 'random-tile-meta' },
              `Перемешиваем из ${totalQuestions} вопросов · все варианты вместе`
            )
          ),
          el('div', { class: 'random-tile-arrow' }, '→')
        ),
        el('div', { class: 'random-row-secondary' },
          el('button',
            { class: 'btn-mini', type: 'button', onclick: () => startRandomQuiz(20) },
            '20 случайных'
          ),
          el('button',
            { class: 'btn-mini', type: 'button', onclick: () => startRandomQuiz(60) },
            '60 случайных'
          ),
          el('button',
            { class: 'btn-mini', type: 'button', onclick: () => startRandomQuiz(totalQuestions) },
            'Все ' + totalQuestions + ' подряд'
          ),
        )
      ),
  ...
  function startQuiz(variantNum) {
    ...
    state.randomCount = 0;
    // Each question is wrapped with its source variant for the review screen
  ...
  function startRandomQuiz(count) {
    ...
    // Pull every question from every variant, tagged with its source.
    const pool = [];
    ...
    const shell = el('div', { class: 'quiz-shell' },
      el('div', { class: 'quiz-header' },
        el('div', { class: 'quiz-meta' },
          ...
          state.isRandom
            ? el('strong', { class: 'random-badge' }, '🎲 Микс')
  ...
  function renderResults() {
    ...
    const card = el('section', { class: 'result-card' },
      el('div', { class: 'result-eyebrow' }, eyebrowText),
        ...
        state.isRandom
          ? el('button',
              { class: 'btn btn-primary', type: 'button',
                onclick: () => startRandomQuiz(state.randomCount) },
              '🎲 Новый микс'
            )
          : el('button',
              { class: 'btn btn-primary', type: 'button',
                onclick: () => startQuiz(state.variantNum) },
              '↻ Пройти этот вариант снова'
            ),
        el('button',
          { class: 'btn btn-ghost', type: 'button', onclick: renderVariantSelect },
          state.isRandom ? 'К вариантам' : 'Другой вариант'
        ),
        el('button',
  ...
  function buildReviewItems() {
    return state.answers.map((a) => {
      ...
      return el('div', { class: cls },
        ...
        state.isRandom && q._fromVariant
          ? el('div', { class: 'review-source' }, 'из варианта №' + q._fromVariant)
          : null,
